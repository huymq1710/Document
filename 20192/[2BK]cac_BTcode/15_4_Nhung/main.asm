...
MAIN:
	...
	MOV 48h, #NUM_8
	MOV 49h, #NUM_9
;Code 43-56: TH sô <0 hay >9 thi ko thuc hien nua, khó vì 8051 ko co lenh so sánh ( 8086 có lênh so sánh CMP)
LOOP:
...
CANCEL:	MOV R0, #0		; Khi dó A<0, set no tro lai ve 0  và xuông duoi hien thi	
	JMP DISPLAY
BIGGER10:
	MOV A, R0			; move data to A
	CLR C
...
ALARM:
  ...
  TRU:CALL DIS			;Code dem giam dan gtri tren den LED ve 0
      ...
      CJNE A, #0, TRU
   CALL DIS
   CJNE A, #9, COI	   ;Tai gtri 0, bat CÒI
